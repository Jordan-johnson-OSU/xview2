...
def load_test_img(data_dir, out_dir, use_files):
    logging.info("Started Test Image into numpy")
    images_array = np.array([])

    if use_files & os.path.isfile(out_dir + '/test/images.npy'):
        logging.info("loading np array from files.")
        images_array = np.load(out_dir + '/images.npy')
        logging.info("np array loaded from files.")
    else:
        image_paths = []
        image_paths.extend([(data_dir + "/" + pic) for pic in os.listdir(data_dir)])
        ...
        images_array = np.asarray(images)
        logging.info("array converted to numpy arrays")

        if use_files:
            # Save output file
            np.save(out_dir + '/images.npy', images_array)
            logging.info("np array saved.")

    logging.info("Finished Load Test Image into numpy")
    return images_array
...
predict = model.predict(test_data, batch_size=32,
    verbose=0,
    steps=None,
    callbacks=None,
    max_queue_size=10,
